...
| 全局 frontier 任务分配 | Step 3-9 | support-v2 已让真实 bag 产生 Region；Component/freshness 仍待修复，尚未完成运行验收 |
...
这一层阻断；当前瓶颈见下文 Component 与 freshness 结论。
...
的第一层 support 阻断，但尚未修复 freshness 执行问题。
...
- Step 3-9 support 职责修复已让真实 bag 产生 Region，但完整运行仍需验证 track、eligible edge、
  assignment 和 freshness，不能据此宣称验收完成。
- 下一项优先评审 supported column 的 Component 连通，不直接降低阈值。
