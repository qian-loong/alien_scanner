...
默认合成模式由两个独立 C++ 节点组成，但使用同一个 launch 和 RViz 配置。V2 Demo 节点发布五个
阶段话题，Component Audit Replay 节点发布四个审计阶段话题；RViz Fixed Frame 为 `map`：
...
```text
/frontier_component_audit/stages/audit_overview/markers
/frontier_component_audit/stages/component_rejection/markers
/frontier_component_audit/stages/direction_evidence/markers
/frontier_component_audit/stages/gap_counterfactual/markers
```

两个节点启动时立即发布九个静态场景，默认不周期重发。所有话题使用 reliable + transient-local QoS，
RViz Display 晚加入或重新打开时会收到各自缓存的最后一帧。需要观察重复发布行为时可显式设置
...
- `support_anchor`：当前显示的 direction attempt 选定的唯一 anchor，以小球标出；
- `support_inward_direction`：从 direction attempt 的实际 anchor 沿 unknown 反方向指向已知空间内部；
- `support_evidence_ray`：从实际 anchor 到最后一个已记录 sample 的单体素证据链；
- `support_known_samples`：ray 上实际检查为 known-free 的 OctoMap 采样位置；
...
`support_inward_direction` 从 direction attempt 的实际 anchor 出发，沿 unknown 反方向指向 inward，
长度等于 `support_depth`，不进行避让偏转。`support_evidence_ray` 固定 Z、单体素宽，并只延伸到 Trace
最后一个已记录 sample；所有 sample 必须位于同一 cardinal key ray。采样点颜色来自对应 OctoMap
状态，而不是固定制造一块示意体素云。
...
V2 Demo 节点同时发布一个标准几何参考阶段和四个真实 replay 阶段：
...
RViz 的顶层 MarkerArray Display 分别控制 Stage 0–8，默认只打开
...
V3 的 Stage 5–8 来自独立的 `frontier_component_audit_replay` 节点。它读取仓库内由真实
`/global_map` bag analyzer 导出的 frame 3 小型快照，不启动 bag reader、不重跑 Detector，也不修改
V2 节点。Stage 5 是统计图；Stages 6–8 是 membership 的 XY 平面审计图，不能解读为 V2 的同一张
OctoMap。为让不同位置的代表案例在 V2 固定相机下可读，Stages 6–8 分别把自身局部重心放到同一显示
中心；重定位不改变阶段内部的列间距、方向票数、阈值或反事实结果，但不同 V3 阶段之间的绝对坐标
不可比较。V3 四个阶段应按阶段切换判读，同时打开只用于比较图层语义，不能解释为空间叠加。
...
support_evidence_ray
support_known_samples
```

这一阶段应能直接观察 anchor、完整要求深度的 inward 箭头、实际访问到的 evidence ray，以及每个采样
位置的地图状态和首个失败点。
...
### 4.6 Component Audit Replay（Stage 5–8）

```text
Stage 5: audit_overview
Stage 6: component_rejection
Stage 7: direction_evidence
Stage 8: gap_counterfactual
```

- `audit_overview`：frame 3 的 `108` 个 Component 和 `2,519` 个 supported XY column 的精确分类与
  column mass；`min_columns`、direction 和 accepted 使用不同颜色；
- `component_rejection`：显示一个 `15` 列 accepted Component，以及 `10` 列和 `6` 列的
  `min_columns` rejects，并在下方给出生产阈值 `12` 的列数标尺；
- `direction_evidence`：显示最小的 direction-rejected Component（C16）的 `+X/+Y/-X/-Y`
  vote arrows，并标出 `90/246 = 0.366 < 0.65`；箭头长度是 vote 比例示意，不是飞行向量；
- `gap_counterfactual`：显示 C26/C37 的一列离散间隔。pair-only 合并会通过现有阈值，但对全部
  radius-2 连通组做传递合并时 frame 3 的 accepted group 从 `1` 变为 `0`；该阶段是只读反事实，
  不把缺口补成 column，也不改变生产 Detector。

membership 快照只有 XY column key，没有逐列 Z 点，因此 Stage 6–8 有意显示为扁平 XY footprint，并在
画面标签中标注这一边界。完整来源仍是 `rosbags/swarm_3_9_20260716_091008` 的 analyzer 输出；快照
不是新的算法输入真值。
...
- 独立 `frontier_component_audit_replay` 读取 frame 3 的确定性组件快照，生成 Stage 5–8 四个
  transient-local MarkerArray；V2 的检测和地图构造代码不参与该节点；
...
当前 Demo 的数据链、阶段语义和 ROS 接线已完成；自动化结果与 evidence-ray-v2 的最新人工验收状态
以 QA 文档第 4 节为准。

V3 的 ROS-free 解析、守恒校验、pair-only/radius-2 反事实和九阶段话题接线已完成；V3 的 RViz 外观
仍需单独进行人工验收，不应把本地 CTest 通过等同于视觉验收通过。

当前 Detector support 已从三维 swept-body envelope 改为单体素 inward evidence ray：按 direction 的
上中位 Z 和同 Z 下 `(x,y)` 字典序最小 raw key 选择 anchor，ray 上任一 unknown/occupied/out-of-bounds
首失败即拒绝。Stage 3 的 fixed-hop 时间语义未改变；新 support 只改变 Stage 4 和独立
`support_evidence` fixture。提交内的 `support-envelope-v1` MCAP 保留修改前画面，用于与当前版本并排
比较。

独立 `vertical_columns`、`support_evidence` 和 `component_fragmentation` 模式仍保留固定教学 fixture，
...
前后两个 support 版本均已录制为五阶段小型 MCAP：

- 修改前：[`frontier-geometry-demo-support-envelope-v1`](bags/frontier-geometry-demo-support-envelope-v1/README.md)；
- 修改后：[`frontier-geometry-demo-support-evidence-ray-v2`](bags/frontier-geometry-demo-support-evidence-ray-v2/README.md)。

两者都使用默认 `combined`、seed `42`、`selected_phi_degrees=0` 和 `republish_rate_hz=0`，五个阶段
话题各包含一条消息。具体文件大小和 SHA-256 记录在各自 README 中。
...
```text
ws/src/swarm_controller/config/frontier_component_audit_frame3_components.csv
ws/src/swarm_controller/config/frontier_component_audit_frame3_membership.csv
```

它们分别包含 frame 3 的 `108` 行 Component detail 和 `2,519` 行 normalized membership（均不含表头）。
快照由完整 analyzer 输出按 `frame_index=3` 机械提取，保留原始 bag/map timestamp；运行期不依赖原始
1.7 GiB bag。当前文件 SHA-256 为：

```text
components:  dac365fc1f3be7d51fd31e17ff0c729ed445aaa7bebc15d6bd35f9bb356aaa3c
membership: 3fe7460b46e1eb1298fe145f5197cb8fcd88e3260535b7b5d6afff557532a264
```

提取来源的完整输出哈希和分析命令仍以 `docs/phases/phase-03-swarm.md` 为准。快照解析会拒绝 schema、
排序、timestamps、membership、完整性或生产阈值不一致的文件。
...
- 每个方向的 Support attempt 独立，最终箭头、evidence ray 和 samples 必须来自同一个 selected attempt；
...
- Stage 5–8 能从同一 frame 3 快照还原 Component 分类、`min_columns` 拒绝、方向票数和 gap 反事实；
...
以下 1-9 项已在当前工作区完成，包括自动化测试和最终 RViz 人工目检。
...
- 默认 launch 提供九个 reliable + transient-local 阶段话题，每个话题一个 publisher；V2 节点共发布
  `36,820` 个场景 Marker；bag 另含每阶段一个 `DELETEALL`；Stages 1–4 的初始构造只执行一次共享 replay；
- V3 frame 3 资产通过 ROS-free 测试校验为 `108` Components / `2,519` columns / `122` gap pairs，
  其中 `6` 个 pair-only counterfactual 通过、radius-2 传递合并 `0` 个 accepted group；
- `gpt-5.6-sol` 修复后窄范围代码复核：`no findings`；
- envelope-v1 的 RViz 五阶段外观已于 2026-07-17 通过人工目检；evidence-ray-v2 于 2026-07-18
  使用同一 Orbit 配置、同一窗口尺寸和单次 bag 回放完成 Stage 4 前后对照，标签与主要几何关系通过。
- v1/v2 五阶段视觉基线均已录制为 5-message MCAP，`ros2 bag info` 校验通过。
- V3 的自动化和运行期缓存验证已通过；V3 RViz 固定视角人工验收待完成，不能沿用 v1/v2 的视觉结论。
